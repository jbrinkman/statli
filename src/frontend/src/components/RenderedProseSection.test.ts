    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.record({
          ...
          // Use simple alphanumeric content to avoid markdown escape sequences and HTML entities
          content: fc.stringMatching(/^[a-zA-Z0-9 ]+$/).filter(s => s.length > 0 && s.length <= 50),
          ...
          return fc.tuple(fc.constant(markdown), fc.constant(htmlTag), fc.constant(content));
        }),
